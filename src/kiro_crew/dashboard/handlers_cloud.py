...
async def api_cloud_preflight(request: web.Request) -> web.Response:
    ...
    return web.json_response({**reach, "session_manager_plugin": bool(plugin)})
...
async def api_cloud_launch_cancel(request: web.Request) -> web.Response:
    ...
    if ev is not None:
        ...
        job.signin = None
        job.error = "Cancelled — no setup was running for this job on this gateway."
        await _in_executor(store.save, job)
...
async def _mutate_instance(request: web.Request, op: str) -> web.Response:
    ...
    sync_teardown = bool(getattr(request.app["state"], "cloud_launch_sync", False))
